...
int main(){
	print("\n\rHello from Main...");
	taskTable[0] = (TaskInfo){6, 4, task1, 0, WAITING, 0}; //Create TCBs and enter in the TCB table
	taskTable[1] = (TaskInfo){5, 5, task2, 1, WAITING, 0}; //release time, execution time, pointer to task function
	taskTable[2] = (TaskInfo){0, 7, task3, 2, WAITING, 0}; //task initial state, elapsed execution time (0)
	initScheduler(&myscheduler,NPCS);
	while(1);
}
